...
const toneClass = computed(
    () =>
        ({
            learn: 'bg-sky-50 text-sky-700 ring-sky-200 dark:bg-sky-500/10 dark:text-sky-300 dark:ring-sky-500/30',
            mini_quiz:
...
<template>
    <div class="rounded-[1.6rem] border border-border/60 bg-card p-5 shadow-sm">
        ...
        >
            ...
            <div
                class="flex flex-col items-start gap-3 rounded-[1.4rem] bg-muted/40 p-4 lg:min-w-[220px]"
            >
                <div
                ...
                <Link
                    :href="item.href"
                    class="inline-flex rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90"
                >
                    Buka detail
                </Link>
            </div>
